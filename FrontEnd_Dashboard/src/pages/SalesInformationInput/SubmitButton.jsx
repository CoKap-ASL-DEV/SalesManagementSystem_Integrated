...
export default function SubmitButton2(props) {
  ...
  const [addFormData] = useMutation(SUBMIT_FORM_QUERY, {
    refetchQueries: [{ query: GET_TABLE }],
  });
  const [uploadFile] = useMutation(UPLOAD_FILE_QUERY);
    ...
    <Button
      onClick={() => {
        ...
        const TotalBuyPrice_Won =
          RewardType === '처분'
            ? parseFloat(TotalSellPrice_Won) * parseFloat(PurchaseRatio) * 0.01
        ...
        const TotalBuyPrice_Dlr =
          RewardType === '처분'
            ? parseFloat(TotalSellPrice_Dlr) * parseFloat(PurchaseRatio) * 0.01
        ...
        addFormData({
          variables: {
            ...
            TotalTechFare:
              parseFloat(TotalBuyPrice_Won) * parseFloat(TechRatio) * 0.01,
            Difference_Dlr:
              parseFloat(TotalSellPrice_Dlr) - parseFloat(TotalBuyPrice_Dlr),
            Difference_Won:
              parseFloat(TotalSellPrice_Won) - parseFloat(TotalBuyPrice_Won),
          },
        });
